    ...
    <>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <button onClick={() => setCurrentDate(new Date(year, month - 1, 1))} className="icon-btn"><ChevronLeft size={20} /></button>
          <span style={{ fontWeight: '600', fontSize: '1rem', minWidth: '160px', textAlign: 'center' }}>{monthLabel}</span>
          <button onClick={() => setCurrentDate(new Date(year, month + 1, 1))} className="icon-btn"><ChevronRight size={20} /></button>
        </div>
        <button onClick={openNew} className="btn btn-primary"><Plus size={15} /> Add Transaction</button>
      </div>
          ...
          {monthlyTransactions.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {monthlyTransactions.map((t) => {
                const d = t.date?.toDate ? t.date.toDate() : new Date(t.date);
                return (
                  <div key={t.id} onClick={() => openEdit(t)} className="card-row" style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', borderRadius: '10px' }}>
          ...
          <div className="modal">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div>
                ...
                <input type="number" placeholder="0.00" value={form.amount} min="0" step="0.01" onChange={(e) => setForm({ ...form, amount: e.target.value })} className="input" style={{ fontSize: '1.2rem', fontWeight: '600' }} autoFocus />
              </div>
              <div>
              ...
              <div>
                ...
                <input placeholder="e.g. Grocery run" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="input" />
              </div>
              <div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '4px' }}>
                {editId ? <button onClick={remove} className="btn btn-danger"><Trash2 size={14} /> Delete</button> : <div />}
